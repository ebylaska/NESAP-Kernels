...
void dgemm_omp_task(char transa, char transb, int m, int n, int k, double alpha, double* a, int lda, double* b, int ldb, double beta, double* c, int ldc, int depth) {
...
double task_recursion_cutoff_level = (log2(omp_get_num_threads()) + 3 ) ;
	...
	//printf("after actual gemm call\n");
		...
		if (n >= max(m,k)) {
#pragma omp task final(depth >= task_recursion_cutoff_level-1) mergeable
			dgemm_omp_task(transa, transb, m, n/2, k, alpha, a, lda, b, ldb, beta, c, ldc, depth+1);                                                            // C0 = alpha * A*B0 + beta * C0
#pragma omp task final(depth >= task_recursion_cutoff_level-1) mergeable
			dgemm_omp_task(transa, transb, m, n-n/2, k, alpha, a, lda, opB ? b+n/2 : b+(n/2)*ldb, ldb, beta, c+(n/2)*ldc, ldc, depth+1);                        // C1 = alpha * A*B1 + beta * C1
#pragma omp taskwait
		} else if (m >= k) {
#pragma omp task final(depth >= task_recursion_cutoff_level-1) mergeable
			dgemm_omp_task(transa, transb, m/2, n, k, alpha, a, lda, b, ldb, beta, c, ldc, depth+1);                                                            // C0 = alpha * A0*B + beta * C0
#pragma omp task final(depth >= task_recursion_cutoff_level-1) mergeable
			dgemm_omp_task(transa, transb, m-m/2, n, k, alpha, opA ? a+(m/2)*lda : a+m/2, lda, b, ldb, beta, c+m/2, ldc, depth+1);                              // C1 = alpha * A1*B + beta * C1
#pragma omp taskwait
		} else {
			dgemm_omp_task(transa, transb, m, n, k/2, alpha, a, lda, b, ldb, beta, c, ldc, depth);                                                         // C  = alpha * A0*B0 + beta * C
			dgemm_omp_task(transa, transb, m, n, k-k/2, alpha, opA ? a+k/2 : a+(k/2)*lda, lda, opB ? b+(k/2)*ldb : b+k/2, ldb, (double)1., c, ldc, depth); // C += alpha * A1*B1
		}
	}
}
...
//	dgemm(transa, transb, m, n, k, alpha, a, lda, b, ldb, beta, c, ldc); 
	...
	dgemm_omp_task(*transa, *transb, *m, *n, *k, *alpha, a, *lda, b, *ldb, *beta, c, *ldc, 0);                                                         
}
